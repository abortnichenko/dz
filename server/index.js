...
app.post('/managers', async (request, response) => {
  ...
  response.status(200).json({
    info: `Manager '${request.body.managerName}' was successfully added`,
  });
});
...
app.post('/managers/:managerId/clients', async (request, response) => {
  ...
  managers[managerId].clients.push({ name, reason });
  await writeData(managers);

  response.setHeader('Content-Type', 'application/json');
  response.status(200).json({
    info: `Client '${name}' was successfully added in clientlist '${managers[managerId].managerName}'`,
  });
});

app.patch(
  '/managers/:managerId/clients/:clientId',
  async (request, response) => {
    const { newClientName, newClientReason } = request.body;
    const managerId = Number(request.params.managerId);
    const clientId = Number(request.params.clientId);

    managers[managerId].clients[clientId].name = newClientName;
    oldClientReason = managers[managerId].clients[clientId].reason;
    managers[managerId].clients[clientId].reason = newClientReason;
    await writeData(managers);

    response.setHeader('Content-Type', 'application/json');
    response.status(200).json({
      info: `'Reason ${oldClientReason} was changed to ${newClientReason}'`,
    });
  }
);
...
app.delete(
  ...
  async (request, response) => {
    ...
    managers[managerId].clients = managers[managerId].clients.filter(
      (task, index) => index !== clientId
    );
    await writeData(managers);
...
app.patch('/managers/:managerId', async (request, response) => {
  ...
  if (destManagerId < 0 || destManagerId >= managers.length) {
    ...
    response
      .status(403)
      .json({ error: `Wrong destination manager ID: '${destManagerId}'` });
  }

  const movedClient = managers[managerId].clients[clientId];
  managers[managerId].clients = managers[managerId].clients.filter(
    (client, index) => index !== clientId
  );
  ...
  response
    .status(200)
    .json({ info: `Client '${movedClient}' was successfully moved` });
});
